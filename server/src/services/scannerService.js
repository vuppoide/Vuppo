...
function analyzeCode(files) {
  ...
  for (const item of files) {
    const filename = item.filename || item.file || 'snippet.txt';
  ...
  return {
    projectName: 'Snippet / Direct Scan',
    filesScanned,
...
const scannerService = {
  scanFiles(files, account = null) {
    if (!Array.isArray(files) || files.length === 0) {
      throw new Error('Nenhum arquivo enviado para análise.');
    }

    const report = analyzeCode(files);

    if (account) {
      authService.recordUsage(account, {
        credits: report.analyzedFiles,
        label: `Auditoria de ${report.analyzedFiles} arquivos`,
        filesScanned: report.analyzedFiles
      });
